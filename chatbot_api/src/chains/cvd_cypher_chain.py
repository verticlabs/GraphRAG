...
from langchain.chains import GraphCypherQAChain #No custom
...
properly (e.g. [a:OWNS] instead of [:OWNS]). Do not run any
queries that would add to or delete from
the database. Make sure to alias all statements that follow as with
statement (e.g. WITH e as entity, e.`Full Name` as full_name)
If you need to divide numbers, make sure to
filter the denominator to be non zero.

Example queries for this schema:

Question: Which is the Entity that has the highest value of the sum of PAID metrics.
cypher query:
MATCH (e:Entity)
UNWIND keys(e) AS key
WITH e, key
WHERE toLower(key) CONTAINS "paid"
WITH e, sum(coalesce(toFloat(e[key]), 0)) AS paidSum
RETURN e
ORDER BY paidSum DESC
LIMIT 1

question: Who are the top 3 entities that owns more Authors?
cypher query:
MATCH (e:Entity)-[:OWNS]->(a:Author)
WITH e, COUNT(a) AS ownersCount
RETURN e, ownersCount
ORDER BY ownersCount DESC
LIMIT 3

Question: Who is the entity that owns the domain with more organit_monthly_etv?
cypher query:
MATCH (e:Entity)-[:OWNS]->(d:Domain)
RETURN e
ORDER BY d.organit_monthly_etv DESC
LIMIT 1
...
When names are provided in the query results, such as persons or institutions names,
beware of any names that have commas or other punctuation in them.
For instance, 'Jones, Brown and Murray' is a single person,
not multiple persons. Make sure you return any list of names in a
